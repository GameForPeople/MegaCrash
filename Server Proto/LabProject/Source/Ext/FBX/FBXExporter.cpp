...
void FBXExporter::ProcessGeometry(FbxNode* inNode)
{
	if (inNode->GetNodeAttribute())
	{
		...
		case FbxNodeAttribute::eMesh:
			...
			mOutputDataContainer[mProcessGeometryCount].mMeshName = inNode->GetName();
...
{
	FbxMesh* currMesh = inNode->GetMesh();
	//std::cout << inNode->GetName() << std::endl;
	...
	for (auto& p : mOutputDataContainer)
	{
		...
		for (unsigned int i = 0; i < p.mTriangles.size(); ++i)
		{
			for (unsigned int j = 0; j < 3; ++j)
			{
				...
				if (FindVertex(p.mVertices[i * 3 + j], uniqueVertices) == -1)
				{
					uniqueVertices.push_back(p.mVertices[i * 3 + j]);
				}
			}
		}
