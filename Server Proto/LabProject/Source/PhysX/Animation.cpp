...
void CSkeleton::Init(FBXExporter & LoadedAnimation, UINT mesh_idx, bool bSaveToFile)
{
	...
	if (bSaveToFile)
	{
		//std::string file_name = "./ServerAssets/Animations/Skeletons/" + LoadedAnimation.mGenericFileName + std::to_string(mesh_idx) + ".sbox";
		std::string file_name = "./ServerAssets/" + LoadedAnimation.mGenericFileName + std::to_string(mesh_idx) + ".sbox";
		FILE* fp = nullptr;
		int error = fopen_s(&fp, file_name.c_str(), "wb");
...
void CSkeleton::Init(const char* strFileName)
{
	FILE* fp = nullptr;
	...
	, arrKeyFrames{ nullptr }
{
	...
	if (bSaveToFile)
	{
		std::string file_name = "./ServerAssets/" + LoadedAnimation.mGenericFileName + std::to_string(mesh_idx) + ".abox";
		FILE* fp = nullptr;
...
{
	FILE* fp = nullptr;
	int error = fopen_s(&fp, strFileName, "rb");
	error = fread_s(strName, sizeof(char) * NAME_LENGTH, sizeof(char), NAME_LENGTH, fp);
	error = fread_s(&nKeyFrames, sizeof(UINT), sizeof(UINT), 1, fp);
	arrKeyFrames = new KeyFrame[nKeyFrames];
	for (int i = 0; i < nKeyFrames; ++i)
	{
		arrKeyFrames[i].arrKeyFrameBones = new KeyFrameBone[nBones];
